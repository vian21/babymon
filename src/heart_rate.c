...
static const uint32_t VALUE_FALLBACK_MAX_MS = 30000; /* extended: window is 20 s */
...
static uint8_t max30102_iic_read(uint8_t addr, uint8_t reg,
                                  uint8_t *buf, uint16_t len); /* fwd decl */
...
static bool sensor_init(void)
{
    ...
    bool ok =
        max30102_set_mode(&s_dev,
                          MAX30102_MODE_SPO2)                          == 0 &&
        max30102_set_spo2_sample_rate(&s_dev,
                          MAX30102_SPO2_SAMPLE_RATE_100_HZ)            == 0 &&
        max30102_set_adc_resolution(&s_dev,
                          MAX30102_ADC_RESOLUTION_18_BIT)              == 0 &&
        max30102_set_spo2_adc_range(&s_dev,
                          MAX30102_SPO2_ADC_RANGE_4096)                == 0 &&
        ...
        max30102_set_fifo_roll(&s_dev,
                          MAX30102_BOOL_TRUE)                          == 0 &&
    ...
    ESP_LOGI(TAG, "MAX30102 ready  (HR window: %u samples = %.0f s)",
             SAMPLE_COUNT, (float)SAMPLE_COUNT / EFFECTIVE_SAMPLE_RATE);
    return true;
}
...
void task_max30102_monitor(void *pv)
{
    ...
    while (true) {
        ...
        /* ── 1. Finger gate (cached state — no FIFO peek) ───────────────── */
        if (!s_finger_present) {
        ...
        fifo_flush();
        vTaskDelay(pdMS_TO_TICKS(20)); /* let fresh samples arrive post-flush */
        ...
        if (!hr_valid(hr) || !spo2_valid(spo2)) {
            if (now_ms - last_warn_ts > WARN_LOG_PERIOD_MS) {
                if (!hr_valid(hr))
                    ESP_LOGW(TAG, "[%lu ms] Invalid HR: %.1f bpm",
                             now_ms, hr);
                if (!spo2_valid(spo2))
                    ESP_LOGW(TAG, "[%lu ms] Invalid SpO2: %.1f%%",
                             now_ms, spo2);
                last_warn_ts = now_ms;
            }
            vTaskDelay(pdMS_TO_TICKS(100));
