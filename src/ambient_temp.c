...
static esp_err_t ambient_i2c_init(void) {
    ...
    ESP_RETURN_ON_ERROR(
        i2c_driver_install(AMBIENT_I2C_PORT, config.mode, 0, 0, 0),
        TAG,
...
static float bme680_compensate_temperature(uint32_t raw_temp) {
    float var1 =
        (((float)raw_temp / 16384.0f) - ((float)s_calib.par_t1 / 1024.0f)) *
        (float)s_calib.par_t2;
    float var2 =
        (((float)raw_temp / 131072.0f) - ((float)s_calib.par_t1 / 8192.0f));
...
static esp_err_t bme680_read_temperature(float* temperature_c) {
    ...
    ESP_RETURN_ON_ERROR(
        ambient_i2c_read(BME680_FIELD0_ADDR, field_data, sizeof(field_data)),
        TAG,
...
static bool ambient_temperature_out_of_range(float temperature_c) {
    return temperature_c < AMBIENT_TEMP_MIN_C ||
    ...
    for (;;) {
        ...
        if (err == ESP_OK) {
            ...
            if (ambient_temperature_out_of_range(temperature_c)) {
                control_hvac(
                    TEMPERATURE_MEASUREMENT, temperature_c, args->wanted_value);
            }
        } else {
            ESP_LOGE(TAG,
